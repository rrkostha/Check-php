...
$extensao = strtolower(end(explode('.',$_FILES['imagemproduto']['name'])));


if (array_search($extensao, $_UP['extensoes']) === false) {

  echo "<META HTTP-EQUIV=REFRESH CONTENT = '0; URL=http://index.php'><script type=\"text/javascript\">alert(\"Por favor envie aqruivo com as seguintes extensões JPG - JPEG -PNG -GIF.\");</script>;";
...
else if ($_UP['tamanho'] < $_FILES['imagemprodutos']['size']) {
...
//mantem o nome
 $nomefinal = $_FILES['imagemproduto']['name'];
  ...
  if(move_uploaded_file($_FILES['imagemproduto']['tmp_name'], $_UP['pasta'].$nomefinal))  {
   ...
   $query ="INSERT INTO produto (nome,descricao_curta,descricao_longa,tag,description,categoria_id,created,imagem) VALUES ('$nomeproduto','$descricaocurta','$descricaolonga','$tag','$description','$categoria',now(),'$imagem')";
   ...
   echo "<META HTTP-EQUIV=REFRESH CONTENT = '0; URL=http://localhost/cliente/listaproduto.php'><script type=\"text/javascript\">alert(\"Produto cadastrado com Sucesso.\");</script>;";
         ...
         echo "<META HTTP-EQUIV=REFRESH CONTENT = '0; URL=http://index.php'><script type=\"text/javascript\">alert(\"Produto não cadastrado.\");</script>;";
