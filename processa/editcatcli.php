...
$query ="UPDATE categoriacliente set nome='$nome',modified=NOW() WHERE id='$id'";
mysqli_query($link,$query) or die('Erro ao alterar os dados '.mysqli_error($link));
		$linhas2=mysqli_affected_rows($link);
		if ($linhas2>=1) {	
echo "<META HTTP-EQUIV=REFRESH CONTENT = '0; URL=http://localhost/admin/listacatclientes.php'><script type=\"text/javascript\">alert(\"Categoria alterada com Sucesso.\");</script>;";
